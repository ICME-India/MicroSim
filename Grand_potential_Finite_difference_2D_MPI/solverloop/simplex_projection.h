...
void projection_on_simplex(double *phi, double *deltaphi, double *divphi) {
  long a, b;
  Deltaphi = 0.0;
  count_phases = 0;
  ...
  for (a=0; a < NUMPHASES; a++) {
    if ((fabs(divphi[a]) > 0.0) && (phi[a] + deltaphi[a]) > 0.0  && (phi[a] + deltaphi[a]) < 1.0) {
      count_phases++;
  ...
  for (a=0; a < NUMPHASES; a++) {
    if ((phi[a] + deltaphi[a]) < 0.0) {
      Deltaphi_alpha = fabs(deltaphi[a] + phi[a]);
  ...
  if (Deltaphi > 0.0) {
    for (b=0; b < NUMPHASES; b++) {
      if (((fabs(divphi[b]) > 0.0)) && ((phi[b] + deltaphi[b]) > 0.0)  && ((phi[b] + deltaphi[b]) < 1.0)) {
