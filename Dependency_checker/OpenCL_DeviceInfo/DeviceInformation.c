...
int main(void)
{
    ...
    for (int i = 0; i < num_platforms; i++)
    {
        ...
        for (int j = 0; j < num_devices; j++)
        {
            ...
            // Get device vendor
            err = clGetDeviceInfo(device[j],  CL_DEVICE_VENDOR , sizeof(string), &string, NULL);
            checkError(err, "Getting device vendor");
            printf("\t\tVendor: %s\n", string);
            ...
            // Get device version
            err = clGetDeviceInfo(device[j],  CL_DRIVER_VERSION , sizeof(string), &string, NULL);
            checkError(err, "Getting driver version");
            printf("\t\tDriver Version: %s\n", string);

            cl_uint num;
            // Get Max. vendor id
            err = clGetDeviceInfo(device[j], CL_DEVICE_VENDOR_ID, sizeof(cl_uint), &num, NULL);
            checkError(err, "Getting device vendor id");
            printf("\t\tVendor ID: %d\n", num);
            ...
            // Get maximum __constant arguments 
            err = clGetDeviceInfo(device[j], CL_DEVICE_MAX_CONSTANT_ARGS, sizeof(size_t), &size, NULL);
            checkError(err, "Getting maximum __constant arguments");
            printf("\t\tMax __constant arguments: %ld\n", size);
	    
	    // //Get Extensions 
	    size_t op_size;
	    //Get size pf extension array (op_size)
	    err = clGetDeviceInfo(device[j], CL_DEVICE_EXTENSIONS, 0, NULL, &op_size);
	    char* extensions = (char*)malloc(op_size);
	    err = clGetDeviceInfo(device[j], CL_DEVICE_EXTENSIONS, op_size, extensions, &op_size); 
