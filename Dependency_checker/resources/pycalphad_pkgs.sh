...
cond2=`echo "$pycalphad_ver_current == $pycalphad_ver_req" | bc`
 if [ "$cond2" -eq 1 ]; then
      echo "The required Pycalphad Version: $1 exists \e[0;32m STATUS : OK \e[0m"
      echo Pycalphad Version : $pycalphad_version_current >> version_log.txt
 else
      echo "Pycalphad version : "$pycalphad_version_current" exists but the required version is "$1" Please install and try again. \e[1;36m STATUS : WRONG VERSION \e[0m"
  fi
else
