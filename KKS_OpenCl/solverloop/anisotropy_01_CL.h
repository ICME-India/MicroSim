void anisotropy_01_dAdq (double *qab, double* dadq, int a, int b, double *dab); 
double anisotropy_01_function_ac(double *qab, int a, int b, double *dab); 
...
void anisotropy_01_dAdq (double *qab, double* dadq, int a, int b, double *dab) {
  ...
  if (fabs(q2) > 1.0e-15) {
   dadq[X] = 16.0*dab[a*npha+b]*(qx3/q22 - qab[X]*q4/q23);
   dadq[Y] = 16.0*dab[a*npha+b]*(qy3/q22 - qab[Y]*q4/q23);
   dadq[Z] = 16.0*dab[a*npha+b]*(qz3/q22 - qab[Z]*q4/q23);
  } else {
    dadq[X] = 0.0;
    dadq[Y] = 0.0;
    dadq[Z] = 0.0;
  }
  //printf("%le, %le, %le\n", dadq[X], dadq[Y], dadq[Z]);
}
...
double anisotropy_01_function_ac(double *qab, int a, int b, double *dab) {
  ...
  double ac;
  if (fabs(q2) > 1.0e-15) {
   ac = 1.0 - dab[a*npha+b]*(3.0 - 4.0*(qx4 + qy4 + qz4)/(q2*q2));
  } else {
   ac = 1.0;
  }
  //printf("%le\n", ac);
