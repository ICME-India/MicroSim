double calcAnisotropy_01(double *phi_1,
                         ...
                         long phase,
                         double DELTA_X, double DELTA_Y, double DELTA_Z, double *Rot_mat, double *Inv_Rot_mat, int y, int z, int x);
                         ...
                         double DELTA_X, double DELTA_Y, double DELTA_Z, double *Rot_mat, double *Inv_Rot_mat, int y, int z, int x)
{
    ...
    for (ip1 = 0; ip1 < npha; ip1++) {
        for (i = 0; i < 3; i++) {
            for (j = 0; j < 3; j++) {
                for (k = 0; k < 3; k++) {
                    indx = ((ip1*3+i)*3+j)*3+k;
                    phi[ip1][i][j][k] = phi_1[indx];
                    // if (phi_1[indx] > 0.5) {
                    //printf("%d, %d, %d, %d, %d, %d, %d, %d, %le, %le *\n", y, z, x, ip1, i, j, k, indx, phi[ip1][i][j][k], phi_1[indx]);
                    // }
                }
            }
        }
    ...
    //printf("%d, %d, %d, %d, %le, %le, %le *\n", y, z, x, phase, DELTA_X, DELTA_Y, DELTA_Z);
    ...
    for (ip1 = 0; ip1 < npha; ip1++)
    {
        ...
        phiMid[ip1][5] = (phi[ip1][1][1][0] + phi[ip1][1][1][1])/2.0;
        phiMid[ip1][6] = (phi[ip1][1][1][2] + phi[ip1][1][1][1])/2.0;

        //printf("gd %le, %le, %le, %le\n", phiMid[ip1][1], phi[ip1][0][1][1], phi[ip1][1][1][1], phiMid[ip1][0]);
    ...
    for (ip1 = 0; ip1 < npha; ip1++)
    {
        ...
        gradPhiMid[ip1][5][ix] = ((phi[ip1][2][1][0] - phi[ip1][0][1][0])/(2.0*DELTA_X) + gradPhiMid[ip1][0][ix])/2.0;
        gradPhiMid[ip1][6][ix] = ((phi[ip1][2][1][2] - phi[ip1][0][1][2])/(2.0*DELTA_X) + gradPhiMid[ip1][0][ix])/2.0;
        ...
        gradPhiMid[ip1][5][iy] = ((phi[ip1][1][2][0] - phi[ip1][1][0][0])/(2.0*DELTA_Y) + gradPhiMid[ip1][0][iy])/2.0;
        gradPhiMid[ip1][6][iy] = ((phi[ip1][1][2][2] - phi[ip1][1][0][2])/(2.0*DELTA_Y) + gradPhiMid[ip1][0][iy])/2.0;

        gradPhiMid[ip1][0][iz] = (phi[ip1][1][1][2] - phi[ip1][1][1][0])/(2.0*DELTA_Z);
        gradPhiMid[ip1][1][iz] = ((phi[ip1][0][1][2] - phi[ip1][0][1][0])/(2.0*DELTA_Z) + gradPhiMid[ip1][0][iz])/2.0;
        gradPhiMid[ip1][2][iz] = ((phi[ip1][2][1][2] - phi[ip1][2][1][0])/(2.0*DELTA_Z) + gradPhiMid[ip1][0][iz])/2.0;
        gradPhiMid[ip1][3][iz] = ((phi[ip1][1][0][2] - phi[ip1][1][0][0])/(2.0*DELTA_Z) + gradPhiMid[ip1][0][iz])/2.0;
        gradPhiMid[ip1][4][iz] = ((phi[ip1][1][2][2] - phi[ip1][1][2][0])/(2.0*DELTA_Z) + gradPhiMid[ip1][0][iz])/2.0;
        gradPhiMid[ip1][5][iz] = (phi[ip1][1][1][1] - phi[ip1][1][1][0])/(DELTA_Z);
        gradPhiMid[ip1][6][iz] = (phi[ip1][1][1][2] - phi[ip1][1][1][1])/(DELTA_Z);

        //printf("gc %le, %le, %le, %le\n", gradPhiMid[ip1][0][ix], phi[ip1][2][1][1], phi[ip1][0][1][1], DELTA_X);
