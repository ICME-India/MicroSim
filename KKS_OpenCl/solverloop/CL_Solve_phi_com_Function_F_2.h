void CL_Solve_phi_com_Function_F_2() {
  ...
  if ( tstep[0] > nsmooth ) {
    ret = clEnqueueNDRangeKernel(cmdQ, ker_SolverCsClEq_F2, work_dim, NULL, globaldim, NULL, 0, NULL, NULL);
    if (ret!=CL_SUCCESS) {
      printf("ker_SolverCsClEq_F2 enq problem  %d in rank = %d \n",ret, rank);
      exit(1);
    }
  }

  if ( tstep[0] <= nsmooth ) {
    ret = clEnqueueNDRangeKernel(cmdQ, ker_SolverPhi_F2_smooth, work_dim, NULL, globaldim, NULL, 0, NULL, NULL);
  }
  else {
    ret = clEnqueueNDRangeKernel(cmdQ, ker_SolverPhi_F2, work_dim, NULL, globaldim, NULL, 0, NULL, NULL);
  }
  if (ret!=CL_SUCCESS) {
    printf("ker_SolverPhi_F2 enq problem  %d at %ld \n",ret, tstep[0]);
    exit(1);
  }
  ...
  if ( tstep[0] <= nsmooth ) {
    ret = clEnqueueNDRangeKernel(cmdQ, ker_SolverCatr_F2_smooth, work_dim, NULL, globaldim, NULL, 0, NULL, NULL);
  }
  else {
    ret = clEnqueueNDRangeKernel(cmdQ, ker_SolverCatr_F2, work_dim, NULL, globaldim, NULL, 0, NULL, NULL);
  }
  if (ret!=CL_SUCCESS) {
    printf("ker_SolverCatr_F2 enq problem  %d at %ld \n",ret, tstep[0]);
    exit(1);
  }
