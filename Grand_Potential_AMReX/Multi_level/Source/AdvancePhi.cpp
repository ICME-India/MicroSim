...
//####################################################################################################################
void 
AmrCoreGP::update_phi(amrex::MultiFab& phi_new, amrex::MultiFab& phi_old,amrex::MultiFab& term1, amrex::MultiFab& term2, amrex::MultiFab& term3,amrex::MultiFab& term4,amrex::MultiFab& lambad, amrex::GpuArray<Real,AMREX_SPACEDIM> dx, amrex::Real dt, int lev)
{	
	...
	for (MFIter mfi(phi_old); mfi.isValid(); ++mfi)
	{
		...
		// 	if(lev==i){
		// 		Tauu = tau*(2^(finest_level))/(2^(lev));
		// 		epsilon = eps*(2^(finest_level))/(2^(lev));
		// 	}
		// }
		...
		// 		Tauu = tau/4;
		// 	}
		//Print()<<"Step: "<<istep[lev]<<"\n";
		//Print() << "Tauu: " << Tauu << " Epsilon: " << epsilon << " Level: " << lev << "\n";
		//Real Tauu = tau;
		Real time_step = dt;
		//Real epsilon = eps;
		Real molar_vol = Vm;
...
{		
	...
	//Print() << "Advancing phi at level " << lev << " with dx = " << dx[0] << "\n";
    ...
    update_phi(phi_state, temp_phi, temp_term1, temp_term2, temp_term3, temp_term4, lambad_state, dx, dt, lev);
    ...
    if(dim == 2)
	{
		dmudt_2D(mu_state, temp_mu, temp_phi1, temp_phi, comp_state, temp_comp,dx, dt,lev);
    }
    if(dim == 3)
	{
