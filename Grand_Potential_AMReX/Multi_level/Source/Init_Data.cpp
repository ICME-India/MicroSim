...
//####################################################################################################################
void
AmrCoreGP::Init_phi_cyl(amrex::MultiFab& phi_new, int lev, int ref)
{
	for (MFIter mfi(phi_new); mfi.isValid(); ++mfi)
	{	
		const Box& wbx = mfi.validbox();
		...
		for(int p=0; p<cylinder.size(); p++)
		{
			...
			Real cyl_rad    = cylinder[p][5] * scale;
			Real cyl_X_cent = cylinder[p][1] * scale;
			Real cyl_Y_cent = cylinder[p][2] * scale;
			//Real cyl_X_cent = cylinder[p][1]*std::pow(ref,lev);
			//Real cyl_Y_cent = cylinder[p][2]*std::pow(ref,lev);
			//Real cyl_Z_strt = cylinder[p][3]*std::pow(ref,lev);
			//Real cyl_Z_end = cylinder[p][4]*std::pow(ref,lev);
			//Real cyl_rad = cylinder[p][5]*std::pow(ref,lev);
			//Print()<<"My level is-" <<lev<<" Radius = "<<cyl_rad<<"\n";
		    amrex::ParallelFor(wbx, [=] AMREX_GPU_DEVICE( int i, int j ,int k)
		    {	
			    #if (AMREX_SPACEDIM <= 2)
...
void AmrCoreGP::Init_phi_cyl(amrex::MultiFab& phi_new, int lev)
{
    ...
    for (MFIter mfi(phi_new); mfi.isValid(); ++mfi)
    {
        ...
        {
            Real cyl_comp   = cylinder[p][0];
            // Store all these directly in physical units
            Real cyl_X_cent = cylinder[p][1];
            Real cyl_Y_cent = cylinder[p][2];
            Real cyl_rad    = cylinder[p][5];
            ...
            [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                ...
                if ((x-cyl_X_cent)*(x-cyl_X_cent)+ (y-cyl_Y_cent)*(y-cyl_Y_cent)<= cyl_rad*cyl_rad)
                {
                    phiNew(i,j,k,cyl_comp) = 1.0;
                }
            });
