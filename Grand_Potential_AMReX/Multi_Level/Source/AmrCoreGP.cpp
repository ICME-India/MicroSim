...
{   
    ...
    if(cylinder.size()>0)
    {
        //Init_phi_cyl(phi_state,lev,refRatio(lev)[0]);
        Init_phi_cyl(phi_state, lev);
    }
    Init_liq(phi_state);
    if(funcf == 4)
    {
...
{
    MultiFab state1(grids[lev], dmap[lev], mu_new[lev].nComp(),1);
    FillPatch("M",lev, t, state1 , 0, state1.nComp());
    for (int i=0;i<AMREX_SPACEDIM;i++)
        {
            Grad[i].define(grids[lev],dmap[lev],1,0);
        }    
    
    for (MFIter mfi(mu_new[lev]); mfi.isValid(); ++mfi)
...
{   
    ...
    Real maxim = mu_new[0].max(0,0,0);
    Real minim = mu_new[0].min(0,0,0);
    ...
    {   
        ...
        static Vector<Real> muerr{2, 2.5, 3, 3.5, 12};
        Real muthrsd = muerr[lev];

        #if(AMREX_SPACEDIM == 2)
        amrex::ParallelFor(bx,[=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {   
            if (gradx(i,j,k)>muthrsd || grady(i,j,k)>muthrsd)
            {
                tagfab(i,j,k) = tagval;
            }
        });
...
{   for (int lev = finest_level-1; lev >= 0; --lev)
    {   amrex::average_down(phi_new[lev+1], phi_new[lev],geom[lev+1], geom[lev],0, phi_new[lev].nComp(), refRatio(lev));
        ...
        amrex::average_down(comp_new[lev+1], comp_new[lev],geom[lev+1], geom[lev],0, comp_new[lev].nComp(), refRatio(lev));
  }
}
...
AmrCoreGP::InitData()
{
    ...
    for (int lev = 0; lev <= finest_level; lev++)
    {
        ...
        //plot[lev].define(grids[lev],dmap[lev],nump,0);
