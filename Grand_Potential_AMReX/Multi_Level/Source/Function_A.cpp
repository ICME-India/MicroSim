...
void
AmrCoreGP::function_A_00_iso_2D(amrex::MultiFab& term1, amrex::MultiFab& phi_old, amrex::GpuArray<Real,AMREX_SPACEDIM>& dx)
{    
    #ifdef AMREX_USE_OMP
	...
	for(MFIter mfi(phi_old); mfi.isValid(); ++mfi)
	{
		...
		//Iteration----------------------------------------------------------------------------------------------------
		amrex::ParallelFor(bbx,[=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
		{	
			//Defining variables---------------------------------------------------------------------------------------
			...
			for(int a=0; a<numphase; a++)
            {
				for(int b=0; b<numphase; b++)
                {
					if(b!=a)
                    {
						...
						divdAdgradphi += 2.0*gamm(a,b)*(qab(iph,X)*(-(phiOld(i+1,j,k,b)+phiOld(i,j,k,b))*0.5)-qab(imh,X)*(-(phiOld(i,j,k,b)
                        +phiOld(i-1,j,k,b))*0.5))/delta[X] + 2.0*gamm(a,b)*(qab(jph,Y)*(-(phiOld(i,j+1,k,b)+phiOld(i,j,k,b))*0.5)
                        -qab(jmh,Y)*(-(phiOld(i,j,k,b)+phiOld(i,j-1,k,b))*0.5))/delta[Y];																
					}
...
//3-D isotropic function---------------------------------------------------------------------------------- 
void
AmrCoreGP::function_A_00_iso_3D(amrex::MultiFab& term1, amrex::MultiFab& phi_old, amrex::GpuArray<Real,AMREX_SPACEDIM>& dx)
{
    #ifdef AMREX_USE_OMP
    #pragma omp parallel if (Gpu::notInLaunchRegion())
    #endif
...
void
AmrCoreGP::function_A_01_ani_2D(amrex::MultiFab& term1, amrex::MultiFab& phi_old, amrex::GpuArray<Real,AMREX_SPACEDIM>& dx)
{
    #ifdef AMREX_USE_OMP
    #pragma omp parallel if (Gpu::notInLaunchRegion())
    #endif
...
void
AmrCoreGP::function_A_01_ani_3D(amrex::MultiFab& term1, amrex::MultiFab& phi_old, amrex::GpuArray<Real,AMREX_SPACEDIM>& dx)
{
    #ifdef AMREX_USE_OMP
    #pragma omp parallel if (Gpu::notInLaunchRegion())
    #endif
			...
			for(int a=0; a<numphase; a++)
            {
                for(int b=0; b<numphase; b++)
                {
                    if(b!=a)
                    {
						...
						divdAdgradphi1 += 2*gamm(a,b)*(ac(iph)*ac(iph)*qab(iph,X)*(-(phiOld(i+1,j,k,b)+phiOld(i,j,k,b))*0.5)-ac(imh)*ac(imh)*qab(imh,X)*(-(phiOld(i,j,k,b)
                                +phiOld(i-1,j,k,b))*0.5))/delta[X] + 2*gamm(a,b)*(ac(jph)*ac(jph)*qab(jph,Y)*(-(phiOld(i,j+1,k,b)+phiOld(i,j,k,b))*0.5)-ac(jmh)*ac(jmh)*qab(jmh,Y)
                                *(-(phiOld(i,j,k,b)+phiOld(i,j-1,k,b))*0.5))/delta[Y]+ 2*gamm(a,b)*(ac(kph)*ac(kph)*qab(kph,Z)*(-(phiOld(i,j,k+1,b)+phiOld(i,j,k,b))*0.5)
                                -ac(kmh)*ac(kmh)*qab(kmh,Z)*(-(phiOld(i,j,k,b)+phiOld(i,j,k-1,b))*0.5))/delta[Z];
						...
						divdAdgradphi2 += 2*gamm(a,b)*(ac(iph)*q2(iph)*(-(phiOld(i+1,j,k,b)+phiOld(i,j,k,b))*0.5)*r_dadq(iph,X)-ac(imh)*q2(imh)*(-(phiOld(i,j,k,b)
                                +phiOld(i-1,j,k,b))*0.5)*r_dadq(imh,X))/delta[X] + 2*gamm(a,b)*(ac(jph)*q2(jph)*(-(phiOld(i,j+1,k,b)+phiOld(i,j,k,b))*0.5)
                                *r_dadq(jph,Y)-ac(jmh)*q2(jmh)*(-(phiOld(i,j,k,b)+phiOld(i,j-1,k,b))*0.5)*r_dadq(jmh,Y))/delta[Y]+2*gamm(a,b)*(ac(kph)*q2(kph)
                                *(-(phiOld(i,j,k,b)+phiOld(i,j,k+1,b))*0.5)*r_dadq(kph,Z)-ac(kmh)*q2(kmh)*(-(phiOld(i,j,k,b)+phiOld(i,j,k-1,b))*0.5)*r_dadq(kmh,Z))/delta[Z];
					}
				}
