...
{	
	...
	{	//Print()<<"Boundary condition for phi in direction "<<idim<<" is: "<<bc_lo_phi[idim]<<" and "<<bc_hi_phi[idim]<<"\n";
		//Print()<<"Value of int_dir is: "<<BCType::int_dir<<"\n";

		if (bc_lo_phi[idim] == 3 && bc_hi_phi[idim] == 3){
            is_periodic[idim] = 1;
			Print()<<"Periodic boundary condition in direction: "<<idim<<"\n";
        }
	}
	...
	MultiFab print(ba,dm,nump+2*(numcom-1)+AMREX_SPACEDIM,0);
	...
	// for (int n = 0; n < bc_phi.size(); ++n) {
	// 	Print() << "phi_Component " << n << "\n";
	...
	// for (int n = 0; n < bc_mu.size(); ++n) {
	// 	Print() << "mu_Component " << n << "\n";
	...
	// for (int n = 0; n < bc_comp.size(); ++n) {
	// 	Print() << "comp_Component " << n << "\n";
	...
	Print()<<"Before initialisation of phi\n";

	//Initialise the function pointers and fill phi-------------------------------------------------------------------------
	init_functions(phi_new);

	Print()<<"After initialisation of phi\n";
	...
	if(GG!=1){
		Calculate_Tau(phi_new,tau_ab);			//Calculate Tau
	}
	else{
		int count=0;
		for(int a=0;a<nump;a++){
			for(int b=a+1;b<nump;b++){
				tau_ab[a][b] = Tau[count];
				tau_ab[b][a] = Tau[count];
				tau_ab[a][a] = Tau[count];
				count++;
			}
		}
	}
	...
	for(n=stepnum+1; n<=nsteps; ++n)
	 {  
		...
		//Filling internal ghost cells for phi_old and mu_old----------------------------------------------------------------------------------


        //Advance the solution to calculate field variables for next time step----------------------------------------------------------------------------------
	 	advance(phi_old, phi_new, mu_old, mu_new, comp_old, comp_new, term1, term2, term3, term4, disp_X, disp_Y , disp_Z, psi, lambad, bc_phi, bc_mu, bc_comp, bc_disp, geom);
