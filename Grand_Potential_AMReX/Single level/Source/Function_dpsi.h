	...
	for ( MFIter mfi(phi_old); mfi.isValid(); ++mfi )
    {
        ...
        {	
			...
			for(int a=0; a<numphase; a++){
					...
					c_mu(i,j,k,mu,c,dcdmu_a,BB,AA,numcomp,a);
...
void func_dpsi(amrex::MultiFab& phi_old, amrex::MultiFab& term3){
    ...
    {	
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
			...
			for(int a=0; a<numphase; a++){
				...
				term(i,j,k,a) = -(lat_ht*(temp-temp_eq)/temp_eq)*dhphi(i,j,k,phiOld, numphase-1, a,numphase);
