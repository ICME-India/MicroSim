...
{       Print()<<"Inside readC()\n";
        ...
        for(int a=0; a<nump-1;a++){
            ...
            temprt[a].resize(cval.size());
            
         
            for(int r=0; r<numcom-1;r++){
                for(int b=0; b < cval.size(); b++)
                {   
...
double findC_sol(int phase, double temp, int i)
{   
    Print()<<"inside sol\n";
...
double findC_liq(int phase, double temp, int i)
{   
    Print()<<"inside liq\n";
    A_accel_ptr = gsl_interp_accel_alloc();
    A_spline_ptr = gsl_spline_alloc(gsl_interp_cspline, conc_Liq[phase].size());

    
    double x_array[conc_Liq[phase].size()];
    double y_array[conc_Liq[phase].size()];

    for (int m=0; m < conc_Liq[phase].size(); m++)
    {
        x_array[m] = temprt[phase][m];
        y_array[m] = conc_Liq[phase][m][i];

    }
    gsl_spline_init(A_spline_ptr, x_array, y_array, conc_Liq[phase].size());
    double y = gsl_spline_eval(A_spline_ptr, temp, A_accel_ptr);
    Print()<<"done liq\n";
    return y;
...
void getc(){
    ...
    Print()<<"in getc files\n";
    conc_s = Vector<Vector<Real>>(nump,Vector<Real>(numcom-1,0.0));
    conc_l = Vector<Vector<Real>>(nump,Vector<Real>(numcom-1,0.0));
    conceq_s = Vector<Vector<Real>>(nump,Vector<Real>(numcom-1,0.0));
    conceq_l = Vector<Vector<Real>>(nump,Vector<Real>(numcom-1,0.0));

    for(int a=0; a<nump-1; a++){
        for(int i=0; i<numcom-1; i++){
                
            conc_s[a][i] = findC_sol(a,T,i);
            conc_l[a][i] = findC_liq(a,T,i);
            conceq_s[a][i] = findC_sol(a,Teq,i);
            conceq_l[a][i] = findC_liq(a,Teq,i);

        } 
    }
    Print()<<"after getc files\n";
...
void function_F_04_function_A(){
    ...
    Print()<<"Before Reading A values from the file\n";

    readA();

    Print()<<"After Reading A values from the file\n";
...
void function_F_04_function_B(){
    ...
    Print()<<"Before calling getc()\n";
    getc();
    Print()<<"After calling getc()\n";
    ...
    for(int a=0; a< nump; a++){
        ...
        for(int i = 0; i< numcom-1; i++){    
            for (int k=0; k < numcom-1; k++) {
                if (k!=i) {
                    sum_c += A[nump-1][k][i]*conc_l[a][k] - A[a][k][i]*conc_s[a][k];
            }
            }
