...
#pragma omp parallel if (Gpu::notInLaunchRegion())
	...
	for ( MFIter mfi(phi_old); mfi.isValid(); ++mfi )
    {
        ...
        {	
			...
			Array1D<Real,0,compcount-2> mu_blk{};
			...
			if(interface==0){
				...
				func_Mu(i,j,k,bulk_phase,numcomp,compn,mu_blk,AA,BB);

				for(int l=0; l<numcomp-1; l++){
					mun(i,j,k,l) = mu_blk(l);
				}
			...
			if(numcomp==2){
				...
				//mun(i,j,k,0) = muo(i,j,k,0) + (divflux(0) -sum_fin(0))*inv_denom(0,0);

				compn(i,j,k,0) = compo(i,j,k,0) + (divflux(0) -divjat(0));
				//compn(i,j,k,0) = compo(i,j,k,0) + (divflux(0));
			}
