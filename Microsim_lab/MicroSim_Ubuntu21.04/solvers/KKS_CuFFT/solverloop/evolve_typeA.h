void evolve_FFT(char *argv[])
{
    ...
    cufftDoubleComplex *gradPhi_x, *gradPhi_y, *gradPhi_z;
    ...
    cudaMalloc((void**)&gradPhi_x, sizeof(cufftDoubleComplex)*MESH_X*MESH_Y*MESH_Z);
    cudaMalloc((void**)&gradPhi_y, sizeof(cufftDoubleComplex)*MESH_X*MESH_Y*MESH_Z);
    cudaMalloc((void**)&gradPhi_z, sizeof(cufftDoubleComplex)*MESH_X*MESH_Y*MESH_Z);
    ...
    while (count <= initcount+numsteps && loop_condition == 1)
    {
        ...
        //gradient of phi
        ComputeGradphi_FFT<<< Gridsize, Blocksize >>>(dfdphiHost[1],
                                                      gradPhi_x, gradPhi_y, gradPhi_z,
                                                      kx, ky, kz,
                                                      MESH_X, MESH_Y, MESH_Z);

        if (cufftExecZ2Z(plan, gradPhi_x, gradPhi_x, CUFFT_INVERSE) != CUFFT_SUCCESS)
            printf("inverse fft of gradPhi_x failed\n");
        if (cufftExecZ2Z(plan, gradPhi_y, gradPhi_y, CUFFT_INVERSE) != CUFFT_SUCCESS)
            printf("inverse fft of gradPhi_y failed\n");
        if (cufftExecZ2Z(plan, gradPhi_z, gradPhi_z, CUFFT_INVERSE) != CUFFT_SUCCESS)
            printf("inverse fft of gradPhi_z failed\n");

        Normalize<<< Gridsize, Blocksize >>>(gradPhi_x, sizescale, MESH_X, MESH_Y, MESH_Z);
        Normalize<<< Gridsize, Blocksize >>>(gradPhi_y, sizescale, MESH_X, MESH_Y, MESH_Z);
        Normalize<<< Gridsize, Blocksize >>>(gradPhi_z, sizescale, MESH_X, MESH_Y, MESH_Z);

        //Driving forces: varmob (saved in gradphi) and dfdphi
        ComputeDrivForce_FFT<<< Gridsize, Blocksize >>>(compHost[0], phiHost[1],
                                                        dfdphiHost[1],
                                                        gradPhi_x, gradPhi_y, gradPhi_z,
                                                        f0AVminv, f0BVminv,
                                                        ceq[0][0][0], ceq[1][1][0],
        ...
        if (cufftExecZ2Z(plan, gradPhi_x, gradPhi_x, CUFFT_FORWARD) != CUFFT_SUCCESS)
            printf("forward fft of gradPhi_x failed\n");
        if (cufftExecZ2Z(plan, gradPhi_y, gradPhi_y, CUFFT_FORWARD) != CUFFT_SUCCESS)
            printf("forward fft of gradPhi_y failed\n");
        if (cufftExecZ2Z(plan, gradPhi_z, gradPhi_z, CUFFT_FORWARD) != CUFFT_SUCCESS)
            printf("forward fft of gradPhi_z failed\n");

        //dfdc
        ComputeDfdc_FFT<<< Gridsize, Blocksize >>>(dfdcHost[0],
                                                   gradPhi_x, gradPhi_y, gradPhi_z,
                                                   kx, ky, kz,
                                                   MESH_X, MESH_Y, MESH_Z);
        ...
        if (cufftExecZ2Z(plan, compHost[0], compHost[0], CUFFT_INVERSE) != CUFFT_SUCCESS)
            printf("inverse fft of comp failed\n");
        if (cufftExecZ2Z(plan, phiHost[1], phiHost[1], CUFFT_INVERSE) != CUFFT_SUCCESS)
            printf("inverse fft of phi failed\n");
