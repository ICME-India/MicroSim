...
//####################################################################################################################
void
AmrCoreGP::Vec_rot(int i, int j, int k, int a, int b, 
            Array2D<Real,0,AMREX_SPACEDIM*2,0,AMREX_SPACEDIM-1,Order::C> &vec, 
            Array2D<Real,0,AMREX_SPACEDIM*2,0,AMREX_SPACEDIM-1,Order::C> &r_vec, 
...
void 
AmrCoreGP::Vec_rot(int i, int j, int k, int a, int b,
            Array1D <Real, 0, AMREX_SPACEDIM-1> &vec, 
            Array1D <Real, 0, AMREX_SPACEDIM-1> &r_vec,
            Array2D <Real,0,phasecount-1,0,phasecount-1,Order::C> matrot_x,
            Array2D <Real,0,phasecount-1,0,phasecount-1,Order::C> matrot_y,
            Array2D <Real,0,phasecount-1,0,phasecount-1,Order::C> matrot_z)
{
...
//####################################################################################################################
void 
AmrCoreGP::Inv_vec_rot(int i, int j, int k, int a, int b, 
            Array2D <Real,0,AMREX_SPACEDIM*2,0,AMREX_SPACEDIM-1,Order::C> &vec, 
            Array2D <Real,0,AMREX_SPACEDIM*2,0,AMREX_SPACEDIM-1,Order::C> &r_vec, 
            Array2D <Real,0,phasecount-1,0,phasecount-1,Order::C> matrot_x, 
            Array2D <Real,0,phasecount-1,0,phasecount-1,Order::C> matrot_y, 
...
//####################################################################################################################
void 
AmrCoreGP::mat_inv(Array2D<Real,0,compcount-2,0,compcount-2, Order::C> &denom, Array2D<Real,0,compcount-2,0,compcount-2, Order::C> &inv_denom,int numcomp)
{
    int size = numcomp-1;
    Real fact{};
...
//####################################################################################################################
void 
AmrCoreGP::mat_mul2D(Vector<Vector<Vector<Real>>> &dc_dmu,Vector<Vector<Vector<double>>> &diffu,Vector<Vector<Real>> &prod, int sz) 
{
	 Real sum;
    for(int l=0; l<sz; l++)
    {
...
//####################################################################################################################
void 
AmrCoreGP::mat_mul1D(Vector<Vector<Real>> &inv_dcdmu, Vector<Real> &deltac, Vector<Real> &deltamu, int sz)
{ 
   Real sum;
    for(int i=0; i<sz; i++)
    {
