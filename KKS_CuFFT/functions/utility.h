void devPropQuery()
{
    ...
    for (int i = 0; i < devCount; i++)
    {
        ...
        if (!devProp.concurrentManagedAccess)
            UMFlag = 0;
    }

    if (!UMFlag)
        printf("Unified memory is not supported by utilized GPU\n");
}
...
int checkOverlap(long MESH_X, long MESH_Y, long MESH_Z, long centx, long centy, long centz, double rad, int *occupancy)
{
    if (!shield_dist)
        return 0;
