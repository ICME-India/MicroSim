...
// 	sum_dhphi += dhphi(gridinfo[center].phia, a, b)*grad->deltaphi[b];
        sum_dhphi += dhphi(gridinfo[center].phia, a, b)*gridinfo[center].deltaphi[b];
      }
      for (k=0; k < NUMCOMPONENTS-1; k++) {
	sum[k]       += c_mu(gridinfo[center].compi, T, a, k)*sum_dhphi;
        sum_dcbdT[k] += dcbdT_phase[a][k]*hphi(gridinfo[center].phia, a);
//         sum_dcbdT[k] += func_dcbdT_phase(gridinfo[center].compi, T, a, k)*hphi(gridinfo[center].phia, a);
      }
    }
    for (k=0; k < NUMCOMPONENTS-1; k++) {
      divflux[k] = 0.0;
    ...
    if (interface==0) {
      for (k=0; k < NUMCOMPONENTS-1; k++) {
	c_old[k] = c_mu(gridinfo[center].compi, T, bulk_phase,k);
	c_new[k] = c_old[k] + deltat*(divflux[k])-divjat[k];
      }
      for (k=0; k < NUMCOMPONENTS-1; k++) {
        deltamu[k] = Mu(c_new, T, bulk_phase, k) - gridinfo[center].compi[k];
	gridinfo[center].compi[k] = Mu(c_new, T, bulk_phase, k);
      }
    } else {
      for (k=0; k < NUMCOMPONENTS-1; k++) {
        if (ISOTHERMAL) {
	...
	for (l=0; l < NUMCOMPONENTS-1; l++) {
	  for (a=0; a < NUMPHASES; a++) {
	    dcdmu[k][l] += dc_dmu(gridinfo[center].compi, T, a, k, l)*hphi(gridinfo[center].phia, a);
	  }
	}
      }
      if (BINARY) {
        ...
        gridinfo[center].compi[0] += deltamu[0];
      }
      ...
      if (TERNARY) {
        ...
        vectorsum(deltamu, gridinfo[center].compi, gridinfo[center].compi,NUMCOMPONENTS-1);
      }
      if (DILUTE) {
        for (k=0; k < NUMCOMPONENTS-1; k++) {
          inv_dcdmu[k][k]             =  1.0/dcdmu[k][k];
          deltamu[k]                  =  deltac[k]*inv_dcdmu[k][k];
          gridinfo[center].compi[k] += deltamu[k];
        }
      }
    ...
    for (b=0; b < NUMPHASES; b++) {
//       gridinfo[center].phia[b] = gridinfo[center].phia[b] + grad->deltaphi[b];
       gridinfo[center].phia[b] = gridinfo[center].phia[b] + gridinfo[center].deltaphi[b];
       //FINDING MAXIMUM AND MINIMUM VALUES 
       if (gridinfo[center].phia[b] > global_max_min.phi_max[b]) {
         global_max_min.phi_max[b] = gridinfo[center].phia[b];
       }
       if (gridinfo[center].phia[b] < global_max_min.phi_min[b]) {
         global_max_min.phi_min[b] = gridinfo[center].phia[b];
       }
       global_max_min.rel_change_phi[b] += gridinfo[center].deltaphi[b]*gridinfo[center].deltaphi[b];
       //FINISHED
    }
    for (k=0; k<NUMCOMPONENTS-1; k++) {
        //FINDING MAXIMUM AND MINIMUM VALUES 
       if (gridinfo[center].compi[k] > global_max_min.mu_max[k]) {
         global_max_min.mu_max[k] = gridinfo[center].compi[k];
       }
       if (gridinfo[center].compi[k] < global_max_min.mu_min[k]) {
         global_max_min.mu_min[k] = gridinfo[center].compi[k];
       }
       global_max_min.rel_change_mu[k] += deltamu[k]*deltamu[k];
       //FINISHED
    }
  }
}
void calculate_divergence_concentration_smooth_2D(long x, struct gradlayer **gradient) {
  ...
  for (gidy=1; gidy <=(end[Y]+2); gidy++) {
    ...
    center        =  gidy   + (x)*rows_y;
    front         =  gidy   + (x+1)*rows_y;
    right         =  center + 1;
    left          =  center - 1;
