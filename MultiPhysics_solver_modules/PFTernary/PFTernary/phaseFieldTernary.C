...
//    volVectorField q_6 = 0.0*q;
    ...
    gsl_spline *spline7 = gsl_spline_alloc (gsl_interp_cspline, np);
    gsl_interp_accel *acc7 = gsl_interp_accel_alloc ();
    gsl_spline_init (spline7, T1, cSol1, np);
    
    gsl_spline *spline8 = gsl_spline_alloc (gsl_interp_cspline, np);
    ...
    gsl_spline *spline11 = gsl_spline_alloc (gsl_interp_cspline, np1);
    gsl_interp_accel *acc11 = gsl_interp_accel_alloc ();
    gsl_spline_init (spline11, T2, Lf_arr, np1);

    gsl_spline *spline12 = gsl_spline_alloc (gsl_interp_cspline, np1);
   ...
   //    scalarRectangularMatrix suscept(2,2,0);
    ...
    c_a[0][0] = gsl_spline_eval (spline7, T0.value(), acc7);
    c_a[1][0] = gsl_spline_eval (spline8, T0.value(), acc8);
    
    c_l[0][0] = gsl_spline_eval (spline9, T0.value(), acc9);
    c_l[1][0] = gsl_spline_eval (spline10, T0.value(), acc10);
...
//    transportProperties.set("omega",epsilon*0.18*testMatrix[0][0]);
   ...
   while (simple.correctNonOrthogonal()){
    ...
    } while(InitialResidual_1 > 1e-05 || InitialResidual_2 > 1e-05);
